 ...
 * inspector. Edits the node's name (its on-canvas label) and its attribute bag.
 * Subscribes by id so edits re-render it without re-reading the whole canvas.
 */
export function NodeInspector({ nodeId }: { nodeId: string }) {
  ...
  const setNodeAttr = useEditorStore((s) => s.setNodeAttr);
  const renameNodeAttr = useEditorStore((s) => s.renameNodeAttr);
  const removeNodeAttr = useEditorStore((s) => s.removeNodeAttr);
    ...
    <div className="w-64 rounded-lg border bg-background/95 p-3 shadow-md backdrop-blur">
      ...
      <div className="grid gap-3">
        ...
        <AttributesEditor
          ...
          onSet={(key, value) => setNodeAttr(node.id, key, value)}
          onRename={(oldKey, newKey) => renameNodeAttr(node.id, oldKey, newKey)}
          onRemove={(key) => removeNodeAttr(node.id, key)}
        />
      </div>
    </div>
  );
